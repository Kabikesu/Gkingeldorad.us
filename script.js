...
function updateHero() {
    ...
    if (text) { text.style.transform = `translate3d(0, ${progress * titleMove}px, 0)`; text.style.opacity = 1 - fade; }
    if (treeLeft) { treeLeft.style.transform = `translate3d(${-progress * treeMove}px, ${progress * (mobile ? 12 : 25)}px, 0)`; treeLeft.style.opacity = 1 - clamp(progress * 1.15); }
    if (treeRight) { treeRight.style.transform = `translate3d(${progress * treeMove}px, ${progress * (mobile ? 12 : 25)}px, 0)`; treeRight.style.opacity = 1 - clamp(progress * 1.15); }
    if (gateLeft) { gateLeft.style.transform = `translate3d(${-progress * gateMove}px, 0, 0)`; gateLeft.style.opacity = 1 - clamp(progress * 1.08); }
    if (gateRight) { gateRight.style.transform = `translate3d(${progress * gateMove}px, 0, 0)`; gateRight.style.opacity = 1 - clamp(progress * 1.08); }
}
function requestHeroUpdate() { if (!heroRaf) heroRaf = requestAnimationFrame(updateHero); }
if (hero) { updateHero(); addEventListener('scroll', requestHeroUpdate, { passive: true }); addEventListener('resize', requestHeroUpdate); }
...
if (menuToggle && navigation) {
    ...
    document.addEventListener('click', (event) => { if (!navigation.contains(event.target) && !menuToggle.contains(event.target)) { navigation.classList.remove('open'); menuToggle.setAttribute('aria-expanded', 'false'); } });
}
...
let timer = null;
let playing = true;
const imageTitle = (file) => file.replace(/\.[^.]+$/, '').replace(/[-_]+/g, ' ').replace(/\b\w/g, (character) => character.toUpperCase());
...
function renderGallery(files) {
    ...
    files.forEach((file) => {
        ...
        dot.className = 'dot'; dot.type = 'button'; dot.setAttribute('aria-label', `Show ${name}`); dot.onclick = () => setSlide(dots.indexOf(dot)); dotsBox.appendChild(dot);
    });
    slides = [...galleryMain.querySelectorAll('.mySlides')]; dots = [...dotsBox.querySelectorAll('.dot')];
    if (slides.length) { showSlide(0); start(); } else { if (counter) counter.textContent = '0 / 0'; if (title) title.textContent = 'No images yet'; }
}
async function loadGallery() {
    try {
        const response = await fetch('https://api.github.com/repos/Kabikesu/Gkingeldorad.us/contents/gallery?ref=main', { headers: { Accept: 'application/vnd.github+json' } });
        if (!response.ok) throw Error('Gallery unavailable');
        const files = (await response.json()).filter((file) => file.type === 'file' && /\.(jpe?g|png|webp|gif)$/i.test(file.name)).map((file) => file.name).sort((a, b) => a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' }));
        renderGallery(files);
    } catch (error) { if (counter) counter.textContent = 'Gallery unavailable'; if (title) title.textContent = 'Please try again later'; console.error(error); }
}
function showSlide(slideIndex) {
    if (!slides.length) return;
    index = (slideIndex + slides.length) % slides.length;
    slides.forEach((slide, number) => { slide.style.display = number === index ? 'block' : 'none'; });
...
function start() { stop(); if (slides.length > 1 && playing) timer = setInterval(() => showSlide(index + 1), 4000); }
function setSlide(slideIndex) { showSlide(slideIndex); start(); }
prev?.addEventListener('click', () => setSlide(index - 1));
next?.addEventListener('click', () => setSlide(index + 1));
play?.addEventListener('click', () => { playing = !playing; play.textContent = playing ? 'Ⅱ' : '▶'; play.setAttribute('aria-label', playing ? 'Pause automatic slideshow' : 'Play automatic slideshow'); play.setAttribute('aria-pressed', playing); playing ? start() : stop(); });
...
function updatePlatformCounters() {
    document.querySelectorAll('[data-platform-count]').forEach((element) => { element.textContent = platforms.length; });
    document.querySelectorAll('[data-platform-count-label]').forEach((element) => { const count = platforms.length; element.textContent = `${count} ${count === 1 ? 'PLATFORM' : 'PLATFORMS'}`; });
}
...
if (platformGrid && !matchMedia('(prefers-reduced-motion: reduce)').matches) {
    platformGrid.addEventListener('pointermove', (event) => {
        ...
        card.style.setProperty('--tilt-x', `${y * -3}deg`); card.style.setProperty('--tilt-y', `${x * 3}deg`); card.style.setProperty('--shine-x', `${((x + 1) / 2) * 100}%`); card.style.setProperty('--shine-y', `${((y + 1) / 2) * 100}%`);
    });
    platformGrid.addEventListener('pointerout', (event) => {
        const card = event.target.closest('.platform-card');
        if (!card || card.contains(event.relatedTarget)) return;
        card.style.setProperty('--tilt-x', '0deg'); card.style.setProperty('--tilt-y', '0deg'); card.style.setProperty('--shine-x', '50%'); card.style.setProperty('--shine-y', '50%');
    });
}

function createAmbientParticles() {
    if (document.querySelector('.ambient-particles') || matchMedia('(prefers-reduced-motion: reduce)').matches) return;
    const layer = document.createElement('div');
    layer.className = 'ambient-particles'; layer.setAttribute('aria-hidden', 'true');
    ...
    layer.appendChild(fragment); document.body.appendChild(layer);
}
...
function addPromotionStyles() {
    ...
    style.textContent = `
        ...
        .promotion-image { display:block; width:100%; max-height:78vh; object-fit:contain; border-radius:12px; }
        .promotion-open-link { display:flex; align-items:center; justify-content:center; width:100%; margin-top:10px; padding:11px 16px; border:1px solid var(--bubble-accent,#e5bd52); border-radius:10px; background:linear-gradient(135deg,color-mix(in srgb,var(--bubble-accent,#e5bd52) 32%,transparent),rgba(255,255,255,.06)); color:#fff; font-weight:800; text-decoration:none; letter-spacing:.04em; transition:transform .2s ease,filter .2s ease,box-shadow .2s ease; }
        .promotion-open-link:hover { filter:brightness(1.15); transform:translateY(-1px); box-shadow:0 0 22px color-mix(in srgb,var(--bubble-accent,#e5bd52) 45%,transparent); }
        .promotion-close { position:absolute; top:18px; right:18px; z-index:2; display:grid; place-items:center; width:38px; height:38px; border:1px solid rgba(255,255,255,.25); border-radius:50%; background:rgba(0,0,0,.55); color:#fff; font-size:1.25rem; cursor:pointer; }
...
function setBubblePath(button,bubbleIndex){
    const vw=Math.max(window.innerWidth,320), vh=Math.max(window.innerHeight,500), size=window.innerWidth<=520?48:60;
    const maxX=Math.max(0,vw-size-8), maxY=Math.max(0,vh-size-8), padX=Math.min(24,maxX/3), padY=Math.min(24,maxY/3);
    const x=()=>randomBetween(padX,Math.max(padX,maxX-padX)), y=()=>randomBetween(padY,Math.max(padY,maxY-padY));
...
function createPromotionSystem(promotions){
    ...
    function renderPromotion(promotionIndex){
        ...
        const url=promotion.url || getPromotionUrl(promotion.file);
        ...
        modal.style.setProperty('--bubble-accent',getPromotionAccent(promotion.file));
        openLink.href=url || '#';
        openLink.textContent=url ? `OPEN ${promotionName(promotion.file).toUpperCase()} →` : 'PLATFORM LINK UNAVAILABLE';
        openLink.style.display=url ? 'flex' : 'none';
    }
    function openPromotion(promotionIndex=0){ renderPromotion(promotionIndex); overlay.classList.add('open'); overlay.setAttribute('aria-hidden','false'); document.body.classList.add('promotion-open'); closeButton.focus(); }
...
async function loadPromotions(){
    try{
        const response=await fetch(promotionApi,{headers:{Accept:'application/vnd.github+json'}});
        if(!response.ok)throw Error('Promotion folder unavailable');
        const files=await response.json();
        const promotions=files.filter((file)=>file.type==='file'&&/\.(jpe?g|png|webp|gif)$/i.test(file.name)).map((file)=>({file:file.name,url:getPromotionUrl(file.name)}));
        createPromotionSystem(promotions);
    }catch(error){ console.error('Promotions unavailable:',error); }
}
loadPromotions();